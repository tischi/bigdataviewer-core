...
public class MultiVolumeRendererShaderSource extends AbstractShaderSource{
	...
	private String[] fragmentShaderCode(){
		...
		String[] body ={
				...
				"		if("+suvActiveVolumes+"[i]==1 &&all(greaterThanEqual(normtexturePos,vtextOffsets[i]))&&all(lessThanEqual(normtexturePos, vtextOffsets[i] +vtextScales[i]))){",
				"			float value;",
				"			if("+suvUseGradient+"==0){",	
				...
				"	    vtextOffsets[n] = vec3(1.0/(2.0*tmp.x),1.0/(2.0*tmp.y),1.0/(2.0*tmp.z));",
				"	    vtextScales[n] = (vec3("+suvVoxelCount+"[n]-ivec3(1)))/vec3("+suvVoxelCount+"[n]);",
				"	 	"+sgvSampleSize+"[n] ="+suvMaxDiagonalLength+"[n]/float("+suvSamples+");",
				"",   
				"    	int csteps = "+stepsFunction.call(new String[]{""+sgvSampleSize+"[n]","ray_pos","ray_dir"})+";",
				"    	if(steps < csteps){",
				"    		steps = csteps;",
				"  		}",    
				"		csteps = "+stepsToVolume.call(new String[]{""+sgvSampleSize+"[n]","ray_pos","ray_dir"})+";",
				"    	if(startStep < csteps){",
				"    		startStep = csteps;",
				"  		}",  
				"	}",
				...
				"				maxDensity = nextDensity;",
				"				sliceColor = gamma*vec4(maxDensity,maxDensity,maxDensity, 1.0);",
				"				sliceColor.a*=abs(latestdDistanceToSlice);",
				"				for(int n = 0; n < "+scvMaxNumberOfVolumes+";n++){",
				"					"+sgvRayPositions+"[n] += "+sgvRayDirections+"[n]*abs(latestdDistanceToSlice);",
				"				}",
				"				renderedSlice = 1;",
				"			}",
