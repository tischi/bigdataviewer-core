...
public class MultiVolumeRendererShaderSource extends AbstractShaderSource{
	...
	private String[] fragmentShaderCode(){
		...
		String[] body ={
				...
				"	for(int i = 0; i < "+scvMaxNumberOfVolumes+"; i++){",
				"		vec3 normtexturePos=positions[i]*vtextScales[i] + vtextOffsets[i]; ",
				...
				"			float value;",
				"			if("+suvUseGradient+"==0){",	
				"				value = texture("+suvVolumeTexture+"[i], positions[i]*vtextScales[i] + vtextOffsets[i] ).r;",	
				"			}else{",
				...
				"					maxDensity = max(maxDensity,densities[0]*"+sgvVolumeNormalizeFactor+");",
				"				}",
				"				c_out = gamma*vec4(maxDensity,maxDensity,maxDensity, 0.5);",
				"			}",
