...
public class MultiVolumeRendererShaderSource extends AbstractShaderSource{
	...
	private String[] vertexShaderCode() {
		String[] shaderCode ={
			...
			/*	"	for(int i =0; i<"+scvMaxNumberOfVolumes+"; i++ ){",
				...
				"		"+svRayStartCoordinate+"[i] = transformed.xyz/transformed.w;",
				"	}",*/
				"",
	...
	private String[] fragmentShaderCode(){
		...
		String[] head = {
				...
				"	return correctTexturePositions(transformedPosition.xyz,vtextOffsets[volumeNumber],vtextScales[volumeNumber]);",
				"}",
				"",
				"",
		...
		String[] body ={
				...
				"			}else{",
				"				value = 7.0*length("+gradient.call(new String[]{"positions.xyz", suvVolumeTexture+"[i]",sgvTexTOffsets+"[i]", sgvTexTScales+"[i]"})+".xyz);",
				"			}",
