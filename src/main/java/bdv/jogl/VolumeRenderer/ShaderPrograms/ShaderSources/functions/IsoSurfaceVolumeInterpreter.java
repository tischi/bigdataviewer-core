	...
	public String[] declaration() {
		...
		addCodeArrayToList( new String[]{
			...
			//	"			fFar = fNew;",
				...
				"	iOut += iIn*(constants.y*pow( dot(halfVec,normal)/((length(halfVec)*length(normal))),1.0));//specular",
				"	",
				"	return iOut;",
				"}",
				"",		
			...
			//	"			vec3 gradient = "+gradEval.call(new String[]{sgvRayPositions+"[volume]", suvVolumeTexture+"[volume]"})+";",
				...
				"			color += blinnPhongShading(inconstants,vec3(0.5),normalize(gradient),"+sgvRayDirections+"[volume], normalize("+sgvRayPositions+"[volume]-"+suvLightPosition+"[volume]),"+suvLightIntensiy+");",
				"			n++;",	
				"		}",
				"		color /= float(n);",
				"		c.rgb = color.rgb;",
