...
public class IsoSurfaceVolumeInterpreter extends AbstractVolumeInterpreter {
	...
	public String[] declaration() {
		...
		addCodeArrayToList( new String[]{
				...
				"}",
				"",
				"const vec3 inconstants = vec3(0.6,0.1,0.3);",
				...
				"	iOut += iIn*(constants.z*pow( dot(halfVec,normal),1.0));//specular",
				"	",
				"	return normal;//iOut;",
				"}",
				"",		
				...
				"			if("+suvActiveVolumes+"[volume] == 0 ){",
				"				colors[volume].a=1.0;",
				"				continue;",	
				"			}",	
				"			vec4 xNear = vec4(vec3("+sgvRayPositions+" - "+sgvRayDirections+" * "+suvRenderRectStepSize+").xyz,1.0);",
				"			xNear ="+suvTextureTransformationInverse+"[volume] * xNear;",
				"			xNear /= xNear.w;",
				"",
				"			vec4 xFar = vec4("+sgvRayPositions+".xyz,1.0);",
				"			xFar ="+suvTextureTransformationInverse+"[volume] * xFar;",
				"			xFar /= xFar.w;",
				"",
				"			vec4 refined[2] = refineIntersection(vm1,v,xNear.xyz,xFar.xyz,"+sgvNormIsoValue+","+suvVolumeTexture+"[volume], "+sgvTexTOffsets+"[volume], "+sgvTexTScales+"[volume]);",
				"			if(refined[0].a < 0||refined[0].a < 0.0){",
				"				colors[volume].a=-1.0;",
				"				continue;",	
				"			 }",
			...
			//	"			vec3 gradient = "+gradEval.call(new String[]{sgvRayPositions+"[volume]", suvVolumeTexture+"[volume]"})+";",
				...
				"			colors[volume].rgb = blinnPhongShading(inconstants,ambientColor,normalize(gradient.xyz),-1.0*"+sgvRayDirections+", normalize("+suvLightPosition+"[volume]-"+sgvRayPositions+"),"+suvLightIntensiy+");",
