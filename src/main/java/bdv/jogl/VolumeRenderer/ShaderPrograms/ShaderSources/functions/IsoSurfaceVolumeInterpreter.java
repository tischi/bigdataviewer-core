...
public class IsoSurfaceVolumeInterpreter extends AbstractVolumeInterpreter {
	...
	public String[] declaration() {
		...
		addCodeArrayToList( new String[]{
				...
				"				any(greaterThan("+sgvRayPositions+"[volume],"+suvVoxelCount+"[volume]))||",
			...
			//	"			vec3 gradient = "+gradEval.call(new String[]{sgvRayPositions+"[volume]", suvVolumeTexture+"[volume]"})+";",
				...
				"			colors[volume].rgb = blinnPhongShading(inconstants,ambientColor,normalize(gradient.xyz),-1.0*"+sgvRayDirections+"[volume], normalize("+suvLightPosition+"[volume]-"+sgvRayPositions+"[volume]),"+suvLightIntensiy+");",
