	...
	public String[] declaration() {
		...
		addCodeArrayToList( new String[]{
			...
			//	"			vec3 gradient = "+gradEval.call(new String[]{sgvRayPositions+"[volume]", suvVolumeTexture+"[volume]"})+";",
				...
				"			color = max(blinnPhongShading(inconstants,ambientColor,normalize(gradient),"+sgvRayDirections+"[volume], normalize("+sgvRayPositions+"[volume]-"+suvLightPosition+"[volume]),"+suvLightIntensiy+"),color);",
				"			n++;",	
				"		}",
				"		if(n==0){",
				"			color = ambientColor;",
